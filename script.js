...
function initializeEnvelope() {
    ...
    let isOpen = false;
    let petalsCreated = false; // Track if petals already created
    
    // Create rose petals
    function createPetals() {
        if (petalsCreated) return; // Don't create more petals if already created
        
        petalsCreated = true;
        rosePetals.innerHTML = ''; // Clear any existing petals
        
        for (let i = 0; i < 15; i++) { 
            const petal = document.createElement('div');
            petal.className = 'petal';
            petal.style.left = Math.random() * 100 + 'vw';
            petal.style.animationDelay = Math.random() * 1 + 's'; // Increased delay range
    ...
    // Create petals on load (only once)
    createPetals();
}
